...
This RFC introduces rules pertaining to when Upgrades are scrapped. 
...
Characters may only have Upgrades attached to them if they have the available Upgrade slots. 

When Upgrades are played upon a character, events occur in the following order: 

* If the character has the required type(s) of Upgrade slot, the Upgrade is attached to the character, filling those slot(s). 
* If the character now has too many Upgrades in a particular upgrade slot, Upgrades previously on the character in that slot are scrapped until there are no longer too many Upgrades for any Upgrade slot.
* The active player resolves any triggered abilities in the order of their choosing.
* The inactive player resolves any triggered abilities in the order of their choosing.
