...
The attack is divided into three steps. The attack may consume the turn’s required attack, or may use an extra attack supplied by a card.
...
. The attacking player returns to the “3. Attack” phase above, if they have mandatory extra attacks; or they have used only extra attacks thus far this turn, and they choose to return.
...
All-Out Attack was one of the earliest cards revealed by WotC and its language was clearly prototypical. In its original form, it is the only card to refer to a character as a "Transformer". While the latter Energon Edition version updated that to "character", its text is still a poor fit with the concept of an Extra Attack. It is Errata'd to have the following interpretation:
