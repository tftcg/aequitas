...
==== Aequitas Alternative Tiebreaker Recommendation

The Tiebreaker procedure was not updated for Wave 5 and both the health, and the Star count tie-breakers can easily be a negative play experience. An alternative, and recommended, approach is to simply allow unintentional draws to happen. If a cut is occurring, matches after the cut are played without a time limit until a player wins. More specifically, when time runs out, each player continues to play until the end of a turn in which all characters are tapped. If no one has won, the match ends in a draw.
